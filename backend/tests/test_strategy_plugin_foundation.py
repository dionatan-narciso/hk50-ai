...
class ExampleStrategy:
    def __init__(self, name="Example", *, voting_eligible=True):
        self._name = name
        self.metadata = StrategyMetadata(voting_eligible=voting_eligible)
...
class StrategyPluginFoundationTests(unittest.TestCase):
    ...
    def test_strategy_decision_normalises_signal(self):
        decision = StrategyDecision(strategy="Example", signal="buy", reason="Example")
        self.assertEqual(decision.signal, "BUY")

    def test_strategy_decision_rejects_invalid_signal(self):
        with self.assertRaises(ValueError):
            StrategyDecision(strategy="Example", signal="WAIT", reason="Unsupported")

    def test_strategy_decision_validates_optional_confidence(self):
        StrategyDecision(strategy="Example", signal="HOLD", reason="Valid", confidence=0)
        StrategyDecision(strategy="Example", signal="HOLD", reason="Valid", confidence=100)
        with self.assertRaises(ValueError):
            StrategyDecision(strategy="Example", signal="HOLD", reason="Invalid", confidence=101)

    def test_structural_strategy_protocol_accepts_plugin(self):
        strategy = ExampleStrategy()
        self.assertIsInstance(strategy, Strategy)
        self.assertEqual(
            strategy.evaluate(MarketContext(symbol="HK50", timeframe="1h", data={"rsi": 42})).signal,
            "BUY",
        )
